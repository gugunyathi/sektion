...
export const EventCard = ({ event, onOpen, initialActive = false, muted: mutedProp, onMutedChange }: { event: Event; onOpen: (e: Event) => void; initialActive?: boolean; muted?: boolean; onMutedChange?: (m: boolean) => void }) => {
  ...
  const handleMediaUpload = async (
    ...
    // For API-backed events, upload directly to the backend multipart route
    ...
    if (dbId && newMedia.src && newMedia.src.startsWith("blob:")) {
      try {
        ...
        fd.append("media", new File([blob], `${newMedia.kind}-${Date.now()}`, { type: blob.type || (newMedia.kind === "video" ? "video/mp4" : "image/jpeg") }));
